"""
process_futures_extended.py
------------------
Core data processing for commodity futures replication.

Reads the cleaned bimonthly panel produced by build_clean_data.py
(commodity_panel_extended.parquet) and computes Short Roll and Excess Holding
returns needed for Tables 1 and 2.

Usage
-----
    ipython ./src/process_futures_extended.py
    from process_futures_extended import build_bimonthly_panel
"""
...
def build_bimonthly_panel(data_dir=DATA_DIR, output_dir=OUTPUT_DIR,
                           save=True):
    """
    Read commodity_panel_extended.parquet (from build_clean_data.py) and compute
    returns panel.

    commodity_panel_extended has columns:
        date, commodity, sector, price_n1, price_n2, price_n3, price_n4

    Saves returns_panel_extended.parquet to data_dir.
    """
    data_dir   = Path(data_dir)
    output_dir = Path(output_dir)

    # commodity_panel_extended.parquet may be in _data/ or _output/
    for candidate in [data_dir   / "commodity_panel_extended.parquet",
                      output_dir / "commodity_panel_extended.parquet"]:
        if candidate.exists():
            panel_path = candidate
            break
    else:
        raise FileNotFoundError(
            "commodity_panel_extended.parquet not found in _data/ or _output/. "
    ...
    if returns.empty:
        ...
        print("  Check that commodity names in commodity_panel_extended match NAME_TO_CODE.")
    ...
    if save:
        out_path = data_dir / "returns_panel_extended.parquet"
