...
The following table reports the annualized mean, standard deviation,
...
Table~\ref{tab:basis_summary} reports the distribution of the log basis by
sector over the full sample. Figure~\ref{fig:basis} shows the evolution of
the median log basis over time by sector.

\input{../_output/summary_stats_basis.tex}
\input{../_output/summary_stats_returns.tex}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{../_output/fig_basis_over_time.png}
  \caption{Median log basis ($\ln F^{(2)}_t - \ln F^{(1)}_t$) by sector
  over March 1986--December 2010. Sectors persistently below zero
  (Energy, Metals) are in structural backwardation and earn positive
  roll returns. Sectors persistently above zero (Grains, Softs) are
  in contango and earn negative roll returns. The figure motivates the
  basis as a cross-sectional sorting variable used in Table~2.}
  \label{fig:basis}
\end{figure}
