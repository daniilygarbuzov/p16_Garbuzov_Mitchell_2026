...
def _save_latex(df_sr, df_eh, output_dir):
    """
    Extended Table 1 LaTeX, same layout as main table:
    3 blocks (Mean, Std Dev, t-stat), 4 n-columns, n=1..4.
    Excess Holding has blank (phantom) entries for n=1.
    """
    ...
    lines = [
        r"\begin{center}",
        r"\captionof{table}{Summary Statistics (Extended Sample)}",
    ...
    for sector in SECTOR_ORDER:
        ...
        # means: n=1 phantom, n=2..4 real
        vals.append(r"\phantom{0.000}")
        for n in [2, 3, 4]:
            mean = row.get(f"mean_ann_n{n}", float("nan"))
            vals.append(_fmt_value(mean, "mean"))

        # std devs: n=1 phantom
        vals.append(r"\phantom{0.000}")
        for n in [2, 3, 4]:
            std = row.get(f"std_ann_n{n}", float("nan"))
            vals.append(_fmt_value(std, "std"))
...
def format_table_1(table_dict, output_dir=OUTPUT_DIR):
    ...
    df_sr.to_csv(output_dir / "table1_short_roll_extended.csv")
    df_eh.to_csv(output_dir / "table1_excess_holding_extended.csv")
